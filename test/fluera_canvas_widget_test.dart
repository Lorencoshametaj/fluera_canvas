...
void main() {
  group('FlueraCanvas widget', () {
    testWidgets('mounts without errors and exposes empty state', (tester) async {
  ...
  group('FlueraCanvas initialBytes', () {
    testWidgets('initialBytes pre-loads strokes before the first paint',
      ...
      source.currentState!.pushStroke(CanvasStroke(
    ...
    testWidgets('corrupt initialBytes leaves canvas empty (no exception)',
      ...
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(
          body: FlueraCanvas(
            key: key,
            initialBytes: Uint8List.fromList(
                const [0xDE, 0xAD, 0xBE, 0xEF, 0, 0, 0, 0]),
            enableNativeLiveStroke: false,
          ),
        ),
      ));
  ...
  group('FlueraCanvas edge cases', () {
    testWidgets('single-point stroke is accepted by pushStroke',
      ...
      key.currentState!.pushStroke(CanvasStroke(
    ...
    testWidgets('undo / redo after pushStroke + pushStrokes batch',
      ...
      state.pushStrokes(List.generate(3, (i) => CanvasStroke(
    ...
    testWidgets('clear empties the scene; undo restores; clear again is no-op',
        (tester) async {
      final key = GlobalKey<FlueraCanvasState>();
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(
