...
void main() {
  ...
  group('CanvasSelection model', () {
    test('empty constants and equality', () {
      ...
      const a = CanvasSelection(ids: <NodeId>{}, bounds: Rect.zero);
  ...
  group('FlueraCanvasState selection API', () {
    ...
    testWidgets('select(NodeId) targets a known stroke node, returns false otherwise',
        (tester) async {
      final key = GlobalKey<FlueraCanvasState>();
      await tester.pumpWidget(pump(key));
      final state = key.currentState!;
      state.pushStroke(makeStroke(1));
      final activeChild = state.activeLayer.children.single as CanvasStrokeNode;
    ...
    testWidgets('selection skips locked + hidden layers', (tester) async {
      ...
      final hits = state.selectInRect(const Rect.fromLTWH(-100, -100, 1000, 1000));
